...
For the public presentation, we can successfully receive keyboard input from a user to move the sprite in the correct direction. We have also successfully implemented collision detection (detecting whether a sprite will collide with a wall if it moves in a particular direction) based on the direction list for the block that the sprite is currently in.
